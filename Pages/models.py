...
class Page(models.Model):
    ...
    0 为与首页相同板式
    """
    name = models.CharField(verbose_name='页面名称', max_length=30, primary_key=True)
...
class NavbarObject(models.Model):
    ...
    导航栏显示设置（不包含登陆后显示的栏目）
    """
    page = models.ForeignKey(Page, verbose_name='页面名称', on_delete=models.CASCADE)
    ...
    def __str__(self):
        return str(self.page) + str('-') + str(self.serial_number) + str('-') + str(self.name)
...
class DisplayImage(models.Model):
    ...
    def __str__(self):
        return str(self.page) + str('img') + str(self.serial_number)
...
class DisplayColumn(models.Model):
    ...
    def __str__(self):
        return str(self.page) + str('col') + str(self.serial_number) + str('-') + str(self.column)
